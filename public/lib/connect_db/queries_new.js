...
function customQuery(query, params) {
    return new Promise((resolve, reject) => {
        ...
        connect_db.query(sqlString, params, function (err, response) {
            if (err) {
                return console.error('error running query', err);
            }
          //    console.log('response:', response);
            if (response.rows.length > 0 || response.rowCount > 0) {
...
function prepareWhereClauseParameterized(cond, startParamCounter = 1) {
    var colString = " WHERE ";
    var params = [];
    var paramCounter = startParamCounter;
        ...
        for (var k in cond) {
            if (k == 'OR') {
                for (var key in cond[k]) {
                    ...
                    if (key != lastk) {
                        if (key.includes("%")) {
                            colString += key.replace("%", '') + "::text ILIKE $" + paramCounter + " OR ";
                            params.push('%' + cond[k][key] + '%');
                        } else if (key.includes("ics")) {
                            colString += "LOWER(" + key.replace("ics", '') + ") = LOWER($" + paramCounter + ") OR ";
                            params.push(cond[k][key]);
                        } else {
                            colString += key + " = $" + paramCounter + " OR ";
                            params.push(cond[k][key]);
                        }
                        paramCounter++;
                    } else {
                        if (key.includes("%")) {
                            colString += key.replace("%", '') + "::text ILIKE $" + paramCounter + " ";
                            params.push('%' + cond[k][key] + '%');
                        } else if (key.includes("ics")) {
                            colString += "LOWER(" + key.replace("ics", '') + ") = LOWER($" + paramCounter + ") ";
                            params.push(cond[k][key]);
                        } else {
                            colString += key + " = $" + paramCounter;
                            params.push(cond[k][key]);
                        }
                        paramCounter++;
                    }
            ...
            if (k == 'AND') {
                for (var key in cond[k]) {
                    ...
                    if (key != lastk) {
                        if (key.includes("%")) {
                            colString += key.replace("%", '') + "::text ILIKE $" + paramCounter + " AND ";
                            params.push('%' + cond[k][key] + '%');
                        } else if (key.includes("ics")) {
                            colString += "LOWER(" + key.replace("ics", '') + ") = LOWER($" + paramCounter + ") AND ";
                            params.push(cond[k][key]);
                        } else {
                            colString += key + " = $" + paramCounter + " AND ";
                            params.push(cond[k][key]);
                        }
                        paramCounter++;
                    } else {
...
function selectQuery({ tablename, data, cond, orderby, limit, joins, offset }) {
    return new Promise((resolve, reject) => {
        ...
        if (cond != undefined) {
            var whereResult = prepareWhereClauseParameterized(cond, paramCounter);
            whereString = whereResult.whereString;
            sqlParams = sqlParams.concat(whereResult.params);
            paramCounter = whereResult.paramCounter;
        }
        ...
        if (limit != undefined) {
            limitString = ` LIMIT $${paramCounter}`;
            sqlParams.push(Object.values(limit)[0]);
            paramCounter++;
        }
        if (offset != undefined) {
            offsetString = ` OFFSET $${paramCounter}`;
            sqlParams.push(Object.values(offset)[0]);
            paramCounter++;
        }
      ...
      //    console.log(sqlParams);
        ...
        // Only pass parameters if we have any
        if (sqlParams.length > 0) {
            connect_db.query(sqlString, sqlParams, function (err, response) {
                if (err) {
                    return console.error('error running query', err);
                }
                // console.log('response:', response);
